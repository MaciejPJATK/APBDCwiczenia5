...
public class AdvancedEmpDeptTests
{
    ...
    {
        var result = Database.GetEmps()
            .Join(Database.GetDepts(), e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d })
            .SelectMany(ed => Database.GetSalgrades()
                .Where(s => ed.e.Sal >= s.Losal && ed.e.Sal <= s.Hisal)
                .Select(s => new { ed.e.EName, ed.d.DName, s.Grade })).ToList();
